...
if False:
    ...
    import pandas
    num_header_lines = 2
    samples = pandas.read_csv(vcf_path, skiprows=num_header_lines, nrows=0, sep="\t", compression="gzip").columns[9:]

    cols = ["#CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "FORMAT"] + samples.tolist()
    dtype = {"#CHROM": str, "POS":numpy.uint32}
    ploidy = 2

    for chunk in pandas.read_csv(vcf_path, sep="\t", header=0, skiprows=num_header_lines, chunksize=1000,
                                 parse_dates=False, compression="gzip", quoting=0, memory_map=True,
        ...
        for row_idx, (_, variant) in enumerate(gts.iterrows()):
            if False:
                alleles_for_variant = variant.str.split(pat="/", expand=True, n=ploidy).astype(numpy.int16).to_numpy().flatten()
                parsed_gts[row_idx] = alleles_for_variant
            elif False:
                alleles_for_variant = array("i", (int(allele) for gt in variant for allele in gt.split("/")))
                parsed_gts[row_idx] = alleles_for_variant
            else:
    ...
    import gzip
    import polars
    table = polars.read_csv(gzip.open(vcf_path, 'rt'), skip_rows=2, separator="\t",
                    try_parse_dates=False, n_threads=1, batch_size=1000, has_header=True)
    table = table.with_columns(gt_pos=polars.col("FORMAT").str.split(by=":").list.eval(polars.element().eq("GT")).list.arg_max())
    samples = table.columns[9:]
    
    gts = table.select(samples).select(polars.col("*").exclude("gt_pos").str.split(':')
                                       .list.get("gt_pos")
                                       .str.split(by="/")
